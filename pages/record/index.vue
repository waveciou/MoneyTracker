...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      // 資料編輯送出
      submitHandler() {
        const result = this.DEEP_CLONE(this.resource);
      ...
      resetHandler() {
        if (this.isRecord === true) {
          if (process.client) {
            const isConfirm = window.confirm('確定要重置所有欄位？');
            if (isConfirm === true) {
              // 新增記帳（重置）
              Object.keys(this.resource).forEach((key) => {
      ...
      createHashtagHandler() {
        if (!this.resource.tags.includes(this.hashtagInput) && this.hashtagInput !== '') {
    ...
    computed: {
      ...
      computePrice: {
        ...
        set(value) {
          const reg = /^[0-9]*$/;
          const verification = !!reg.test(value);
          this.resource.price = verification === true ? value : 0;
        },
      },
      // 主類別列表
      categorieList() {
        return this.categoriesData[this.expenseKeyword].map((item) => ({ id: item.id, name: item.name }));
      },
      // 次類別列表
      subcategorieList() {
