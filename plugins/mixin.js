...
Vue.mixin({
  methods: {
    ...
    GET_CATEGORIES_NAME(payload) {
      const result = this.$store.state.categoriesList.filter((item) => item.id === payload);
    ...
    GET_EXAMPLE_DATA() {
      ...
      const exampleList = [...example.example];
      ...
      exampleList.forEach((item) => {
        ...
        assignItem.time.year = thisYear;
        assignItem.time.month = thisMonth;
      });
